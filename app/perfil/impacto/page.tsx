  ...
  return (
    <div className="min-h-screen bg-primary">
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        {/* Total CO2 Ahorrado - DISEÑO CORREGIDO (Texto legible) */}
        <Card className="bg-gradient-to-br from-green-900 to-green-800 border-none p-8 mb-6 shadow-xl relative overflow-hidden">
          ...
          <div className="text-center relative z-10">
            <p className="text-sm font-medium text-green-100/80 mb-2 uppercase tracking-wider">Total de CO₂ evitado</p>
            <div className="flex items-baseline justify-center gap-2 mb-2">
              <span className="text-7xl font-bold text-white tracking-tight">
                {impacto?.total_co2_ahorrado?.toFixed(1) || 0}
              </span>
              <span className="text-2xl font-medium text-green-200">kg</span>
            </div>
            <p className="text-green-100 font-medium bg-white/10 inline-block px-4 py-1 rounded-full text-sm">
              Gracias a tus {impacto?.total_acciones || 0} acciones sostenibles
            </p>
          </div>
        </Card>
          ...
          {[
            ...
            return (
              <Card key={item.tipo} className="bg-secondary border-secondary p-4 text-center">
                ...
                <p className="text-xl font-bold text-white">{data?.cantidad || 0}</p>
                <p className="text-xs text-primary-foreground/70">{item.label}</p>
                <p className="text-xs text-green-400 mt-1">{(Number(data?.co2_total) || 0).toFixed(1)} kg</p>
              </Card>
            )
          })}
        </div>
